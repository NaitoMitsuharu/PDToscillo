...
1. アプリを起動すると**接続画面**が開きます。**LAN ケーブルを挿す（有線が立ち上がる）と、
   前回の接続先へ自動で接続します**（バインドは「有線を固定」を使用）。接続先は端末に保存され、
   アプリを再起動しても手動入力なしで再接続します。手動で「切断」したときは、
   ケーブルを挿し直すまで自動接続しません。
   初めての場合は「初期設定の手順」を開くと、オシロスコープ側の設定手順と静的 IP の設定例が出ます。
2. **IP アドレス**と**ポート**（初期候補 4000）を入力します。
3. 必要なら「詳細設定」で**ソケットのバインド先**を選びます。PDT-FP1 のように LAN 端子が
   イーサネットテザリング扱いになる端末では `有線を固定` を推奨します。
   - `有線を固定` — **推奨**。eth0 の IP をソースアドレスに固定して確実に有線側へ出します。
     Android が LAN を Ethernet として登録しない端末でも機能します
   - `Ethernet（socketFactory）` / `Ethernet（bindSocket）` — 端末が LAN を Ethernet として
     登録している場合に使えます
   - `システム既定（バインドなし）` — 切り分け用
